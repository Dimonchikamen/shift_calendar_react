...
export const findLastInterval = (events: WorkTime[], date: Date, worktime = { start: "0:00", end: "23:00" }) => {
    const todayEvents = JSON.parse(
        JSON.stringify(events.filter(ev => ev.start.split(" ")[0] === moment(date).format(DATE_FORMAT)))
    );
    if (!todayEvents.length) return worktime;
    ...
    if (lastEventTime < worktime.end) {
        lastInterval.start = lastEventTime;
        lastInterval.end = worktime.end;
        return lastInterval;
    }
